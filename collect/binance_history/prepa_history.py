...
def main(tf):
    source_data = './BTCUSDT_binance2020.csv'
    df = pd.read_csv(source_data)
    df = df.drop_duplicates()

    df['t'] = [pd.to_datetime(x, unit='ms').strftime('%Y-%m-%d %H:%M:%S') for x in df['Time']]
    df['index'] = df['t']
    ...
    df['average_dollar_volume_per_trade'] = df['dollar_volume'] / df['count']
   ...
   # CHANGES %  
    ...
    # LONG SHORT
    df['long_%'] = (df['taker_buy_volume'] / df['volume']) * 100
    ...
    print(df)
    df.to_csv('../../backtest/data/dataset/BTCUSDT_binance2020.csv', index=True)
